...
export default function UserHome() {
  ...
  const gradientColors: [string, string] = darkMode ? ["#1e1e1e", "#121212"] : ["#ffebee", "#ffcdd2"]; // ← new only for dark mode
  const surfaceBg      = darkMode ? "#121212"            : "#fff";         // content + drawer background
  ...
  return (
    <>
      ...
      <View style={styles.root}>
        {/* LinearGradient gets dynamic colors */}
        <LinearGradient colors={gradientColors} style={styles.gradient}>
          ...
          {/* drawer keeps same palette; background switched via surfaceBg */}
          <Animated.View
            style={[
              styles.drawer,
              { transform: [{ translateX }], top: insets.top, backgroundColor: surfaceBg },
            ]}
          >
          ...
          {/* main content surface uses surfaceBg */}
          <View
            style={[
              styles.content,
          ...
          >
            <View style={styles.header}>
              <Text style={styles.headerTitle}>{t("homeOfStudents", "דף הבית של הסטודנטים")}</Text>
            </View>
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>{t("relevantNotices", "הודעות רלוונטיות")}</Text>
...
const styles = StyleSheet.create({
  ...
  overlay: { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 15 },
  menuButton: { position: "absolute", right: 16, zIndex: 30, padding: 4 },
  drawer: {
    position: "absolute", bottom: 0, right: 0, width: DRAWER_WIDTH, zIndex: 20, paddingTop: 16, paddingHorizontal: 16,
    ...Platform.select({ ios: { shadowColor: "#000", shadowOffset: { width: -4, height: 2 }, shadowOpacity: 0.3, shadowRadius: 8 }, android: { elevation: 8 } }),
  },
  drawerTitle: { fontSize: 18, fontWeight: "700", marginBottom: 12, textAlign: "right" },
  ...
  header: { height: 56, flexDirection: "row-reverse", alignItems: "center", paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: "#eee" },
  headerTitle: { fontSize: 20, fontWeight: "700", color: "#b71c1c", textAlign: "right" },
  scrollContent: { padding: 16 },
  section: { marginBottom: 24 },
  sectionTitle: { fontSize: 18, fontWeight: "600", color: "#b71c1c", marginBottom: 8, textAlign: "right" },
  card: { backgroundColor: "#ffebee", borderRadius: 12, paddingVertical: 12, paddingHorizontal: 16, marginBottom: 8 },
