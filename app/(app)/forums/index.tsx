...
export default function ForumsHome() {
  ...
  const [statusFilter,setStat] = useState<"all"|"active"|"closed">("all");
  ...
  const fetchForums = () => {
    ...
    return onSnapshot(
      q,
      snap=>{
        const arr = snap.docs.map(d=>({ id:d.id, ...(d.data() as Omit<Forum,"id">) })) as Forum[];
        setForums(arr); setLoading(false); setRef(false);
      },
  ...
  useEffect(()=>{
    if(!loading) Animated.timing(fade,{toValue:1,duration:400,useNativeDriver:true}).start();
  },[loading]);
    ...
    Alert.alert(
      ...
      [
        { text: t("cancel","בטל"), style:"cancel" },
        {
          text: t("delete","מחק"),
          style:"destructive",
          onPress: async ()=>{ try{ await deleteDoc(doc(db,"forums",forumId)); }
      ...
      <SafeAreaView style={[styles.container,{backgroundColor:SURFACE_BG}]} edges={["top","bottom"]}>
        ...
        <View style={styles.controls}>
          ...
          <FlatList
            ...
            renderItem={({item})=>{
              ...
              return(
                <Pressable
                  onPress={()=>setStat(item.key as any)}
                  style={[
                    styles.filterChip,
                    { backgroundColor: active?PRIMARY:GREY_BG }
                  ]}
        ...
        <Animated.View style={{flex:1,opacity:fade}}>
          <FlatList
            data={filtered}
            keyExtractor={f=>f.id}
            refreshControl={<RefreshControl refreshing={refreshing} onRefresh={fetchForums} tintColor={PRIMARY}/>}
            renderItem={({item})=>
              <ForumRow
                forum={item}
                cardBg={CARD_BG}
...
/* ---------- ForumRow עם Swipe למחיקה (Admin בלבד) ---------- */
function ForumRow({
  forum, cardBg, textPrimary, textSecondary,
  isRTL, isAdmin, onDelete,
}:{
  forum:Forum; cardBg:string; textPrimary:string; textSecondary:string;
  isRTL:boolean; isAdmin:boolean; onDelete:(id:string)=>void;
}){
...
const styles = StyleSheet.create({
  ...
  /* card */
  card:{borderRadius:12,padding:16,marginBottom:12,shadowColor:"#000",
