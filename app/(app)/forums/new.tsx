...
export default function NewForum() {
  ...
  const handleSave = async () => {
    if (!title.trim() || !category) {
      Alert.alert("שגיאה", "חובה למלא כותרת וקטגוריה");
      return;
    }
    if (!user) {
    ...
    try {
      ...
      Alert.alert("שגיאה", e.message || "לא ניתן לשמור פורום");
      setSaving(false);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: SURFACE_BG }]} edges={["top", "bottom"]}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>יצירת פורום חדש</Text>
      ...
      >
        ...
        >
          <Text style={[styles.label, { color: TEXT_PRIMARY }]}>כותרת פורום *</Text>
          <TextInput
          ...
          <Text style={[styles.label, { color: TEXT_PRIMARY }]}>קטגוריה *</Text>
          <View
            ref={pickerRef}
            ...
            >
              <ScrollView nestedScrollEnabled>
                {CATEGORIES.map((c) => (
                  ...
                  >
                    <Text style={[styles.dropdownText, { color: TEXT_PRIMARY }]}>{c}</Text>
                  </Pressable>
                ))}
              </ScrollView>
            </View>
          ...
          <Text style={[styles.label, { color: TEXT_PRIMARY }]}>תיאור (לא חובה)</Text>
          <TextInput
            style={[styles.input, styles.textArea, { borderColor: BORDER_COLOR, backgroundColor: FIELD_BG, color: TEXT_PRIMARY }]}
          ...
          <Pressable
            ...
            disabled={saving}
          >
            {saving ? <ActivityIndicator color={LIGHT_BG} /> : <Text style={styles.saveTxt}>שמור פורום</Text>}
          </Pressable>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
