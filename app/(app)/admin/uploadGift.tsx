...
export default function UploadGiftScreen() {
  ...
  const handleDelete = (id: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: isRTL ? "מחק" : "Delete", style: "destructive", onPress: async () => { await deleteDoc(doc(db, "gifts", id)); fetchGifts(); } },
      ]
    );
  };
  ...
  const renderHeader = () => (
    <View style={[styles.formContainer, { backgroundColor: SURFACE_BG }]}>      
      ...
      <TextInput
        style={[styles.input, { borderColor: uploading ? "#ccc" : ACCENT, color: TEXT_PRIMARY, textAlign: isRTL ? 'right' : 'left' }]}
      ...
      />
      <TextInput
        style={[styles.textArea, { borderColor: uploading ? "#ccc" : ACCENT, color: TEXT_PRIMARY, textAlign: isRTL ? 'right' : 'left' }]}
      ...
      <Animated.View style={{ transform: [{ scale: btnScale }] }}>
        ...
        >
          {uploading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{isRTL ? 'הוסף מתנה' : 'Add Gift'}</Text>}
        </Pressable>
      </Animated.View>
  ...
  return (
    <SafeAreaView edges={["top","bottom","left","right"]} style={[styles.flex, { backgroundColor: SURFACE_BG }]}>      
      <LinearGradient colors={HEADER_GRAD} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[styles.header, { paddingTop: insets.top + 8 }]}>  
        <Pressable onPress={() => router.back()} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={24} color="#fff" />
        </Pressable>
        <Text style={styles.headerTitle}>{isRTL ? 'הוספת מתנה' : 'Upload Gift'}</Text>
        <View style={{ width: 24 }} />
      </LinearGradient>
      <KeyboardAvoidingView style={[styles.flex, { paddingBottom: insets.bottom }]} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <FlatList
          data={gifts}
          ...
          renderItem={({ item }) => (
            <View style={[styles.giftCard, { borderColor: ACCENT }]}>              
              <View style={styles.giftRow}>
                <Text style={[styles.giftName, { color: ACCENT, textAlign: isRTL ? 'right' : 'left' }]}>{item.name}</Text>
                <Pressable onPress={() => handleDelete(item.id)}>
                  <Ionicons name="trash-outline" size={20} color={ACCENT} />
                </Pressable>
              </View>
              <Text style={[styles.giftDesc, { color: TEXT_PRIMARY, textAlign: isRTL ? 'right' : 'left' }]}>{item.description}</Text>
            </View>
          )}
          ListEmptyComponent={<Text style={styles.emptyText}>{isRTL ? 'אין מתנות' : 'No Gifts'}</Text>}
